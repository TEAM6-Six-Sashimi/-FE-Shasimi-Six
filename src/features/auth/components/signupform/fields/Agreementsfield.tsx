...
export const AGREEMENT_ITEMS: AgreementItem[] = [
  {
    ...
    description:
      '서비스 제공을 위해 이름, 생년월일, 전화번호, 이메일, 아이디 등 개인정보를 수집 및 이용하는 것에 동의합니다.',
    required: true,
  },
  {
    key: 'marketing',
    title: '마케팅 수신 동의 (선택)',
    description:
      '사이트에서 제공하는 신규 강의 안내, 이벤트, 프로모션 등 마케팅 정보를 이메일로 수신하는 것에 동의합니다.',
    required: false,
  },
  {
    key: 'emailNotice',
    title: '이메일 수신 동의 (선택)',
    description:
      '학습 독려, D-day 알림, 미진행 강의 리마인드 등 사이트에서 제공하는 동기부여 및 학습 관련 알림 이메일을 수신하는 것에 동의합니다.',
    required: false,
  },
  {
    key: 'aiUsage',
      ...
      {AGREEMENT_ITEMS.map((item) => {
        ...
        return (
          <div key={item.key} className="flex flex-col gap-1.5">
            ...
            <p className="text-[12px] text-[#6A7282] pl-7 leading-relaxed">· {item.description}</p>
          </div>
        );
      })}
    </div>
