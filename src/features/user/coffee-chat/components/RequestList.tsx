...
import { InstructorPendingChat } from '../types';
...
interface RequestListProps {
  requests: InstructorPendingChat[];
}
  ...
  if (requests.length === 0) {
    return (
      <div className="flex-1 flex items-center justify-center">
        <p className="text-[13px] text-[#9CA3AF]">아직 요청이 없습니다.</p>
      </div>
    );
  }
          ...
          <div className="w-full flex items-center gap-3 px-5 py-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[14px] font-bold text-[#1E2125] truncate">
                {request.studentId} - {request.courseTitle}
              </p>
