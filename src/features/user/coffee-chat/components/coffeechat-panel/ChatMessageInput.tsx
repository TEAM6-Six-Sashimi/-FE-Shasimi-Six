...
const ChatMessageInput = forwardRef<HTMLInputElement, ChatMessageInputProps>(
  function ChatMessageInput({ value, onChange, onSubmit, disabled }, ref) {
    ...
    return (
      ...
      >
        <input
          ...
          maxLength={2000}
          className="flex-1 h-11 px-4 rounded-lg border border-[#D1D5DB] bg-[#F9FAFB] text-[13.5px] text-[#1E2125] placeholder:text-[#9CA3AF] outline-none focus:border-[#1E2125] transition-colors"
        />
        <button
          type="submit"
          aria-label="메시지 보내기"
