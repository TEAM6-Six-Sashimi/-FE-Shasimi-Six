...
export default function CourseEditForm({ categories, initialData }: CourseEditFormProps) {
  ...
  const handleField = <K extends keyof CourseEditFormData>(key: K, value: CourseEditFormData[K]) => {
  ...
  const inputCls = 'w-full h-11 px-4 rounded-lg border border-[#D1D5DB] bg-white text-[13.5px] text-[#1E2125] placeholder:text-[#6A7282] outline-none focus:border-[#1E2125] transition-colors';
    ...
    <div className="max-w-3xl mx-auto py-8 px-6">
      <div className="bg-white rounded-2xl shadow-md p-8 flex flex-col gap-8">
        ...
        <section className="flex flex-col gap-5">
          <h2 className="text-[16px] font-bold text-[#1E2125] pb-2 border-b border-[#E5E7EB]">기본 정보</h2>

          <div>
            <label className={labelCls}>강의 제목{requiredMark}</label>
            <input type="text" placeholder="강의 제목을 입력하세요" value={form.title}
              onChange={(e) => handleField('title', e.target.value)} disabled={isLoading} className={inputCls} />
          </div>

          <div>
            <label className={labelCls}>강의 설명{requiredMark}</label>
            <textarea placeholder="강의 설명을 입력하세요" value={form.description}
              onChange={(e) => handleField('description', e.target.value)} rows={5} disabled={isLoading}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Select value={form.category} onValueChange={handleCategoryChange} disabled={isLoading}>
                ...
                <SelectContent position="popper">
                  {categories.map((cat) => (
                    <SelectItem key={cat.name} value={cat.name} className="text-[13px]">{cat.name}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <div>
              ...
              <Select value={form.subCategory} onValueChange={(v) => handleField('subCategory', v)}
                ...
                <SelectContent position="popper">
                  {subCategories.map((sub) => (
                    <SelectItem key={sub.id} value={String(sub.id)} className="text-[13px]">{sub.name}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" placeholder="0" value={form.price}
                onChange={(e) => handleField('price', e.target.value === '' ? '' : Number(e.target.value))}
                min={0} disabled={isLoading} className={inputCls} />
            </div>
            <div>
              ...
              <Select value={form.level} onValueChange={(v) => handleField('level', v)} disabled={isLoading}>
                ...
                <SelectContent position="popper">
                  {LEVELS.map((lv) => (
                    <SelectItem key={lv} value={lv} className="text-[13px]">{lv}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          ...
          <div>
            ...
            <input type="text" placeholder="썸네일 URL을 입력하세요" value={form.thumbnail}
              onChange={(e) => handleField('thumbnail', e.target.value)} disabled={isLoading} className={inputCls} />
          </div>
        </section>

        <section className="flex flex-col gap-4">
          <h2 className="text-[16px] font-bold text-[#1E2125] pb-2 border-b border-[#E5E7EB]">커리큘럼</h2>

          {form.sessions.map((session, idx) => (
            <SessionItem key={session.id} session={session} index={idx}
              canRemove={form.sessions.length > 1} onUpdate={updateSession} onRemove={removeSession} />
          ))}

          <button type="button" onClick={addSession} disabled={isLoading}
        ...
        <div className="flex items-center justify-between pt-2">
          <Button type="button" onClick={() => setConfirmModal({ type: 'save' })} disabled={isLoading}
          ...
          <div className="flex gap-3">
            <Button type="button" onClick={() => setConfirmModal({ type: 'submit' })} disabled={isLoading}
              className="h-11 px-7 bg-[#FF5E5E] hover:bg-[#D14848] text-white font-semibold text-[14px] cursor-pointer disabled:opacity-70">
              {isLoading ? '처리 중...' : '승인 요청'}
            </Button>
            <Button type="button" variant="outline" onClick={() => setConfirmModal({ type: 'cancel' })} disabled={isLoading}
  ...
  return (
    <div className="flex flex-col gap-3 p-5 rounded-xl border border-[#E5E7EB] bg-[#F9FAFB]">
      <div className="flex items-center justify-between">
        ...
        {canRemove && (
          <button type="button" onClick={() => onRemove(session.id)}
      ...
      <div>
        ...
        <input type="text" placeholder="세션 제목을 입력하세요" value={session.title}
          onChange={(e) => onUpdate(session.id, 'title', e.target.value)} className={inputCls} />
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input type="url" placeholder="영상 URL을 입력하세요" value={session.videoUrl}
            onChange={(e) => onUpdate(session.id, 'videoUrl', e.target.value)} className={inputCls} />
        </div>
        <div>
          <label className="block text-[12.5px] font-semibold text-[#1E2125] mb-1.5">
            강의 자료 (선택)
          </label>
          <input type="text" placeholder="자료 URL을 입력하세요" value={session.materialFile}
            onChange={(e) => onUpdate(session.id, 'materialFile', e.target.value)} className={inputCls} />
        </div>
      </div>

      <label className="flex items-center gap-2 cursor-pointer w-fit">
        <input type="checkbox" checked={session.preview ?? false}
