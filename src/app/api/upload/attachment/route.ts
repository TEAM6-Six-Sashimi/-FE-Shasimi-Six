...
export async function POST(req: NextRequest) {
  ...
  const res = await fetch(`${API_BASE_URL}/instructor/files/attachment`, {
    method: 'POST',
    headers: {
  ...
  if (!res.ok) {
    ...
    console.error('attachment upload error:', errorBody);
    return NextResponse.json({ error: '자료 업로드에 실패했습니다.' }, { status: res.status });
  }
